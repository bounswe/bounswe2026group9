...
Phase 1 of `backend/TESTING_ROADMAP.md`. Repository collaborators are
accepted as keyword arguments with a production-module default so unit
tests can pass mocks without monkey-patching the import graph.
"""
...
from app.repositories.protocols import BookmarkRepoProtocol, EventRepoProtocol
...
def create_bookmark(
    ...
    *,
    ...
    existing = bookmarks.get_bookmark(db, user_id, event_id)
    if existing:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Already bookmarked")

    bookmark = bookmarks.insert_bookmark(db, {
        "user_id": user_id,
        "event_id": event_id,
    })
...
def remove_bookmark(
    ...
    *,
    ...
    bookmarks.delete_bookmark(db, user_id, event_id)
...
def list_user_bookmarks(
    ...
    *,
    ...
    bookmark_rows, total = bookmarks.get_bookmarks_by_user(
    ...
    event_ids = [b["event_id"] for b in bookmark_rows]
    bookmark_dates = {b["event_id"]: b["created_at"] for b in bookmark_rows}

    # Batch load event data
    events_data = {}
    for eid in event_ids:
        ev = events.get_event_by_id(db, eid)
        if ev:
            events_data[eid] = ev

    valid_event_ids = list(events_data.keys())
    locations_by_event = events.get_primary_locations_for_events(db, valid_event_ids)
    categories_by_event = events.get_categories_for_events(db, valid_event_ids)
    images_by_event = events.get_primary_images_for_events(db, valid_event_ids)
